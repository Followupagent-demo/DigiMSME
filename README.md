...
- **Home** (`/`) — a full 4-point "Loss Point" journey (Search → Enquiry → Negotiation → Payment, each shown broken then covered) using real screens, not narrated placeholders. The opening search scene rotates through all 15 targeted industries (not just a handful of examples) every 2.2s, in sync across its broken/fixed pair — see `search_presets` in `build_home()`.
...
- **Blogs** (`/blogs/`) — 4 real, sourced posts live now (`BLOG_POSTS` in `data.py`): kirana vs quick-commerce, small exporters vs Bangladesh/Vietnam, local gyms vs franchise chains, and the broader MSME shutdown-crisis piece tying them together. Every stat cites a real source (govt Udyam data, Ministry of MSME Annual Report 2025-26, World Bank, KPMG, Indian Retailer) — same honesty rule as the industry citations. The full 6-per-industry library still lands in V2.
- **Free Tools** — still a "Coming in V2" stub.
- Every industry's fix scene now ends with a concrete outcome line — a rupee figure or a volume stat framed as "this would've gone to a competitor" (e.g. private-school: "₹90,000 annual fee booked — would've gone to another school"). These are illustrative demo figures, not real client results — consistent with the Proposed-approach badges.
